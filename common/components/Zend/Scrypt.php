...
namespace common\components\Zend;
...
abstract class Scrypt
{
    ...
    {
        if ($n == 0 || ($n & ($n - 1)) != 0) {
            throw new InvalidConfigException("N must be > 0 and a power of 2");
        }
        if ($n > PHP_INT_MAX / 128 / $r) {
            throw new InvalidConfigException("Parameter n is too large");
        }
        if ($r > PHP_INT_MAX / 128 / $p) {
            throw new InvalidConfigException("Parameter r is too large");
        }

        if (extension_loaded('Scrypt')) {
            if ($length < 16) {
                throw new InvalidConfigException("Key length is too low, must be greater or equal to 16");
            }
