...
class Token extends Model
{
  ...
  */
  public function __construct($param=null){
    //Create a secret token if needed who will be used to
    //get_derived_secret per userId
    $secretToken=\Yii::getAlias('@storage/secretToken');
    if (!is_dir(dirname($secretToken))){
      mkdir (dirname($secretToken),0755);
    }
    ...
    //grab hash size ...
    $param['hashmod_digest_size']=strlen(hex2bin(hash($param['hashmode'], 'hop', false)));
    //Set params as member
    foreach ($param as $key=>$value){
      $this->{$key}=$value;
    }

    $this->secretToken=file_get_contents($secretToken);
    $this->sigSecret=yii::$app->crypto->hkdf(
      $this->hashmode,
      $this->secretToken,
      $this->hashmod_digest_size,
      null,
      self::HKDF_INFO_SIGNING
    );
  ...
  */
  public function makeToken($data){
    ...
    $sig = $this->get_signature($payload);
  ...
  */
  public function verifyToken($token,$secret=null,$now=null){
    ...
    $expected_sig =$this->get_signature($payload,$secret);
    // is signature valid ?
    if (strcmp($sig, $expected_sig)==0){
      ...
      #  $now can be forced to test token validity without expiration
      if ($now == null){
        $now = $this->decistamp();
      }
      if ($data["expires"] < $now){
  ...
  /**
  * Update secret token
  * update sigSecret property
  * @param string old secret token
  */
  ...
  * @return string hexadecimal formated random salt
  */
  ...
  * @param string decoded_token
  * @return array associative array with payload & sig
  */
  public function extractTokenData($decoded_token){
    ...
    $sig = substr(
      $decoded_token,
      strlen($decoded_token)-$this->hashmod_digest_size,
      strlen($decoded_token)
    );
  ...
  */
  private function get_signature($payload,$secret=null){
    if ($secret==null){
  ...
  *
  * @param string token
  * @return string derivated secret
  */
  public function get_derived_secret($token){
    $decoded_token=     $this->decode_token_bytes($token);
    //sig is ignored
    $tokenData=$this->extractTokenData($decoded_token);
    $payload=json_decode($tokenData['payload']);
    $salt=$payload->salt;
    $info = self::HKDF_INFO_DERIVE.$token;
    $secret = \Crypto\Crypto::HKDF(
      $this->secretToken,
      $this->hashmode,
      $salt,
      $this->hashmod_digest_size,
  ...
  */
  ...
  /**
  * return the complete array after the usage of verifyAssertion
  */
  ...
  */
  public  function createAuthToken($authorization){
    //the only valid audience for the token
    $audience=\Yii::$app->params['publicURI'];
    try{
      if (strpos($authorization,'BrowserID')===false){
        throw new \Exception('assertion not found');
      }
      $assertion=str_replace('BrowserID ','',$authorization);
    ...
    $hdkfEmail=\Crypto\Crypto::HKDF(
      '',
      self::DEFAULT_HASHMOD,
      null,
      self::DEFAULT_DIGEST_SIZE,
      $data['email']
    );
    $userInfo=SyncUser::Find()->Where( [
      'email'=>$hdkfEmail
    ]
    )->One();
  ...
  */
  public function verifyHawk($myHawk=null,$testUrl=null,$method='GET'){
    ...
    //a-ton un HawkId ?
    if ($myHawk==null
    && (!\Yii::$app->request->headers->has('Authorization')
    ...
    try{
      ...
      //pas de validation de l'utilisateur, on pourrais être en création
      if (count($userInfo)!=1){
